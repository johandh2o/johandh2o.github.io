...
$$
\begin{aligned}
    & X = (x_i)_{i=1}^n,\\ x_i\in\mathcal{X} & x_i \text{ context/feature of individual } i\in [n]\\
    & T = (t_i)_{i=1}^n,\\ t_i\in\{0,1\} & t_i \text{ treatment for individual } i\in [n]\\
    & Y^F = (y_i^F)_{i=1}^n,\\ y^F_i\in\mathcal{Y} & y_i^F \text{ factual outcome of individual } i\in [n]
\end{aligned}
$$

Unobserved/latent variables in Rubin's framework
------
We can also define the latent variables in Rubin's framework of potential outcomes, which allow us to define directly the ITE *(Individualized Treatment Effect)*, also knows as the CATE *(Conditional Average Treatment Effect)*, and the ATE $(Average Treatment Effect)$:
